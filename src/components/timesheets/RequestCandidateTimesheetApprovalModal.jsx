...
const RequestCandidateTimesheetApprovalModal = ({ isOpen, onClose, candidate }) => {
    const { user } = useAuth();
    ...
    const [companyName, setCompanyName] = useState(''); // Will need to fetch companies or allow input
    ...
    const yearsOptions = Array.from({ length: 5 }, (_, i) => (currentYear - 2 + i).toString()); // Current year +/- 2
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await apiService.sendTimesheetApprovalRequest(
                candidate.email, // Use candidate's email
                candidate.firstName + ' ' + candidate.lastName, // Use candidate's name
                month,
                year,
                deadlineDate,
                companyName,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Request Timesheet Approval" size="md">
            ...
            {canRequestTimesheetApproval && candidate && (
                <form onSubmit={handleSubmit} className="space-y-4">
                    <p className="text-gray-700">
                        Request timesheet approval for **{candidate.firstName} {candidate.lastName}** (Email: **{candidate.email}**).
                    </p>
                    <div>
...
export default RequestCandidateTimesheetApprovalModal;