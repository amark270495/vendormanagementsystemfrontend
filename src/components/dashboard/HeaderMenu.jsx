...
const HeaderMenu = ({ 
  ...
  filterConfig, 
  onFilterChange, 
  isFirstColumn, // New prop to handle the edge-case alignment
  onClose 
}) => {
  ...
  return (
    <div 
      ref={menuRef}
      /* FIX: 'absolute' places it over the table. 
         'isFirstColumn' ensures it doesn't bleed off the left screen.
         'z-[100]' ensures it stays above table rows.
      */
      className={`
        absolute top-full mt-2 min-w-[260px] bg-white rounded-xl shadow-2xl border border-slate-200 z-[100]
        animate-in fade-in zoom-in-95 duration-150
        ${isFirstColumn ? 'left-0 origin-top-left' : 'right-0 origin-top-right'}
      `}
      onClick={(e) => e.stopPropagation()}
    >
      {/* Sorting Section */}
      <div className="p-2 space-y-0.5">
        <p className="px-3 pt-2 pb-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sort</p>
        <button 
          onClick={() => onSort('ascending')} 
          className="flex items-center w-full px-3 py-2 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 rounded-lg transition-colors group"
        >
      ...
      <div className="border-t border-slate-100 bg-slate-50/50 p-4">
        <div className="flex items-center mb-3">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Filter</p>
        </div>
        ...
        <div className="flex items-center gap-2 mt-5">
          <button 
            onClick={handleClear} 
            className="flex-1 inline-flex items-center justify-center px-3 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-red-600 transition-all active:scale-95"
          >
          ...
          <button 
            onClick={handleApply} 
            className="flex-1 inline-flex items-center justify-center px-3 py-2 text-xs font-bold text-white bg-indigo-600 border border-transparent rounded-lg hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all active:scale-95"
          >
