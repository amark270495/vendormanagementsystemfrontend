 ...
 */
const FormInput = ({ id, label, type = 'text', required = false, fullWidth = false, children, ...props }) => (
    <div className={fullWidth ? 'md:col-span-2' : ''}>
        <label htmlFor={id} className="block text-sm font-medium text-gray-700">
            {label} {required && <span className="text-red-500">*</span>}
        </label>
        {children ? (
            React.cloneElement(children, { ...props, id, name: id, className: "mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" })
        ) : (
            <input 
                type={type} 
                id={id}
...
const UserFormModal = ({ isOpen, onClose, onSave, userToEdit }) => {
    ...
    }, [isOpen, currentUser?.userIdentifier]);
    ...
    // *** THIS IS THE HANDLER FOR TYPING ***
    const handleChange = (e) => {
        // This will now update state WITHOUT re-defining the FormInput component
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={userToEdit ? "Edit User" : "Add New User"} size="2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
                ...
                <FormSection title="Basic Info">
                    <FormInput id="username" label="Username (Email)" type="email" required value={formData.username || ''} onChange={handleChange} disabled={!!userToEdit} />
                    <FormInput id="password" label={userToEdit ? "New Password (Optional)" : "Password"} type="password" required={!userToEdit} value={formData.password || ''} onChange={handleChange} placeholder={userToEdit ? "Leave blank to keep unchanged" : ""} />
                    <FormInput id="firstName" label="First Name" required value={formData.firstName || ''} onChange={handleChange} />
                    <FormInput id="lastName" label="Last Name" required value={formData.lastName || ''} onChange={handleChange} />
                    <FormInput id="middleName" label="Middle Name (Optional)" value={formData.middleName || ''} onChange={handleChange} />
                    <FormInput id="dateOfBirth" label="Date of Birth" type="date" required value={formData.dateOfBirth || ''} onChange={handleChange} />
                </FormSection>
                ...
                <FormSection title="Employment & Role">
                    ...
                    <FormInput id="backendOfficeRole" label="Backend Office Role" required>
                        <select value={formData.backendOfficeRole || ''} onChange={handleChange} required>
                    ...
                    <FormInput id="employmentType" label="Employment Type" required>
                        <select value={formData.employmentType || ''} onChange={handleChange} required>
                            {employmentTypes.map(type => <option key={type} value={type}>{type}</option>)}
                        </select>
                    </FormInput>
                    <FormInput id="dateOfJoining" label="Date of Joining" type="date" required value={formData.dateOfJoining || ''} onChange={handleChange} />
                    <FormInput id="workLocation" label="Work Location" required value={formData.workLocation || ''} onChange={handleChange} />
                    <FormInput id="reportsTo" label="Reports To (Manager)">
                        <select value={formData.reportsTo || ''} onChange={handleChange}>
                            <option value="">Select a Manager...</option>
                            {allUsers
                                .filter(u => u.username !== formData.username)
                                .map(u => (
                ...
                <FormSection title="Contact & Personal Details">
                    <FormInput id="personalMobileNumber" label="Personal Mobile Number" type="tel" required value={formData.personalMobileNumber || ''} onChange={handleChange} />
                    <FormInput id="currentAddress" label="Current Address" required fullWidth>
                        <textarea id="currentAddress" rows="3" value={formData.currentAddress || ''} onChange={handleChange} required />
                    </FormInput>
                    <FormInput id="linkedInProfile" label="LinkedIn Profile URL (Optional)" type="url" value={formData.linkedInProfile || ''} onChange={handleChange} placeholder="https://linkedin.com/in/..." />
                    <FormInput id="bloodGroup" label="Blood Group (Optional)">
                        <select value={formData.bloodGroup || ''} onChange={handleChange}>
                ...
                <FormSection title="Emergency Contact">
                    <FormInput id="emergencyContactName" label="Contact Name" required value={formData.emergencyContactName || ''} onChange={handleChange} />
                    <FormInput id="emergencyContactPhone" label="Contact Phone" type="tel" required value={formData.emergencyContactPhone || ''} onChange={handleChange} />
                    <FormInput id="emergencyContactRelation" label="Relation" required>
                         <select value={formData.emergencyContactRelation || ''} onChange={handleChange} required>
