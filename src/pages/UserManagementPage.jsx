...
const UserManagementPage = () => {
    ...
    const handleSaveUser = async (formData) => {
        ...
        try {
            if (userToEdit) {
                ...
                // We pass the original username (RowKey) and the new formData
    ...
    // Helper to format date strings from YYYY-MM-DD
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {users.map((u) => (
                                        <tr key={u.username} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 font-mono text-xs">{u.employeeCode}</td>
                                            <td className="px-6 py-4"><RoleBadge role={u.backendOfficeRole} /></td>
                                            <td className="px-6 py-4 text-xs">{formatDate(u.dateOfJoining)}</td>
                                            <td className="px-6 py-4 text-xs">{formatDate(u.dateOfBirth)}</td>
                                            <td className="px-6 py-4 flex space-x-2 justify-end">
                                                <button 
                                                    onClick={() => handleEditClick(u)} // Pass the full user object with new fields
