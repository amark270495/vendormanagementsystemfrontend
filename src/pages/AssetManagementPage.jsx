...
// ✅ UPDATED: Helper to get IST date string for initial state
...
const AssetManagementPage = () => {
    ...
    const viewAssetSessions = async (asset, selectedDate = sessionDate) => {
        ...
        try {
            // ✅ Backend now expects a shift-aware date
    ...
    // ✅ NEW: Calculate working hours locally to ensure "Unlock/Resume" are included in IST window
    const formattedWorkingTime = useMemo(() => {
        if (!assetSessions.length) return '0h 0m';

        const startTriggers = ['login', 'unlock', 'resume', 'active'];
        const endTriggers = ['logout', 'logoff', 'lock', 'idle'];
    ...
    const getEventBadge = (action, category) => {
        ...
        if (['login', 'unlock', 'resume', 'active'].includes(actionLower)) 
        ...
        if (['logout', 'logoff', 'lock'].includes(actionLower)) 
    ...
    const filteredSessions = assetSessions.filter(s => {
        ...
        if (activeTab === 'attendance') return cat === 'session' || ['login', 'logoff', 'logout', 'lock', 'unlock', 'resume'].includes(act);
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gradient-to-r from-slate-50 to-white gap-4">
                ...
                <button onClick={fetchData} className="text-sm font-medium px-4 py-2 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-50 transition-all flex items-center">
                    Refresh
                </button>
            </div>
                                ...
                                <tr key={asset.rowKey} className="hover:bg-indigo-50/30 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <button 
                                            ...
                                            className="inline-flex items-center px-3 py-1.5 border border-indigo-100 text-xs font-semibold rounded-md text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition-colors"
                                        >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            {canAssignAssets && asset.AssetStatus === 'Available' && (
                                                <button onClick={() => openModal('assign', asset)} className="text-indigo-600 border border-slate-200 px-3 py-1 rounded hover:bg-slate-50">Assign</button>
                                            )}
                                            {canAssignAssets && asset.AssetStatus === 'Assigned' && (
                                                <button onClick={() => openModal('reassign', asset)} className="text-indigo-600 border border-slate-200 px-3 py-1 rounded hover:bg-slate-50">Reassign</button>
                                            )}
                                            {canManageAssets && (
                                                <>
                            ...
                            <div className="flex flex-col h-full overflow-hidden">
                                <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 rounded-t-2xl flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500 mt-0.5">IST Shift: 19:00 - 04:00</p>
                                    </div>
                                ...
                                <div className="px-6 py-4 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <input 
                                            ...
                                            className="px-3 py-1.5 border border-slate-300 rounded-md shadow-sm text-sm font-medium text-slate-700 bg-slate-50"
                                        />
                                    </div>
                                    <div className="flex items-center px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-lg">
                                        <ClockIcon />
                                        <span className="text-sm text-indigo-900 font-medium">
                                            Active Time: <strong className="text-lg font-black ml-1">{formattedWorkingTime}</strong>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
                                    {loadingSessions ? (
                                        <div className="py-16 text-center text-slate-500">Loading tracking data...</div>
                                        ...
                                        <div className="border border-slate-200 rounded-xl shadow-sm overflow-hidden bg-white">
                                            <table className="min-w-full divide-y divide-slate-200">
                                                <thead className="bg-slate-100">
                                                    <tr>
                                                        <th className="px-5 py-3 text-left text-xs font-bold text-slate-600 uppercase w-32">Event</th>
                                                        <th className="px-5 py-3 text-left text-xs font-bold text-slate-600 uppercase">User</th>
                                                        <th className="px-5 py-3 text-left text-xs font-bold text-slate-600 uppercase">Time (IST)</th>
                                                        <th className="px-5 py-3 text-left text-xs font-bold text-slate-600 uppercase">Notes</th>
                                                    </tr>
                                                </thead>
                                                    ...
                                                    {filteredSessions.map((session, idx) => (
                                                        <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                                                            ...
                                                            <td className="px-5 py-3 text-slate-500">
                                                                {/* Display the IST time saved by the backend */}
                                                                {session.istTimeLogged || new Date(session.eventTimestamp).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata' })}
                                                            </td>
                                ...
                                <div className="px-6 py-4 border-t border-slate-200 bg-white rounded-b-2xl flex justify-end">
                                    <button type="button" onClick={closeModal} className="px-6 py-2.5 text-sm font-bold text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 border border-slate-200 shadow-sm">
                                        Close Window
                                    </button>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleActionSubmit} className="space-y-5">
                                ...
                                {(activeModal === 'assign' || activeModal === 'reassign') && (
                                    <div>
                                        ...
                                        <select required value={modalData.userEmail || ''} onChange={(e) => setModalData({...modalData, userEmail: e.target.value})} className="w-full border-slate-300 rounded-lg p-2.5 border">
                                            <option value="" disabled>-- Select --</option>
                                            {users.map(u => <option key={u.username} value={u.username}>{u.displayName}</option>)}
                                        </select>
                                    </div>
                                )}
                                {activeModal === 'service' && (
                                    <>
                                        <select required value={modalData.isRepair || ''} onChange={(e) => setModalData({...modalData, isRepair: e.target.value})} className="w-full border-slate-300 rounded-lg p-2.5 border">
                                            <option value="" disabled>-- Type --</option>
                                            <option value="false">Service</option>
                                            <option value="true">Repair</option>
                                        </select>
                                        <textarea required rows="3" value={modalData.details || ''} onChange={(e) => setModalData({...modalData, details: e.target.value})} className="w-full border-slate-300 rounded-lg p-2.5 border" placeholder="Details..."></textarea>
                                    </>
                                )}
                                <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-100">
                                    <button type="button" onClick={closeModal} className="px-5 py-2 text-sm font-bold text-slate-600 bg-white border border-slate-300 rounded-lg">Cancel</button>
                                    <button type="submit" disabled={processing} className="px-5 py-2 text-sm font-bold text-white bg-indigo-600 rounded-lg">{processing ? '...' : 'Confirm'}</button>
                                </div>
                            </form>
                        )}
                    </div>
