...
const ProfilePage = () => {
    ...
    const loadInitialData = useCallback(async () => {
        ...
        try {
            ...
            // ✅ FIXED ASSET MAPPING: Checking multiple PascalCase and camelCase variations
            if (assetRes?.data && Array.isArray(assetRes.data)) {
                const assigned = assetRes.data.find(a => 
                    (a.assignedToEmail && a.assignedToEmail.toLowerCase() === user.userIdentifier.toLowerCase()) || 
                    (a.AssetAssignedToEmail && a.AssetAssignedToEmail.toLowerCase() === user.userIdentifier.toLowerCase()) || 
                    (a.AssetAssignedTo && a.AssetAssignedTo === user.userName) ||
                    (a.assignedTo && a.assignedTo === user.userName)
                );
    ...
    }, [user?.userIdentifier, fetchLeaveHistory, user?.userName]);
