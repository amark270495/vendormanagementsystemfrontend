...
// --- ICONS (Defined externally for performance and to fix bugs) ---
...
const ProfileCard = ({ user, isEditing, formData, onFormChange, allUsers, isExpanded, onToggleExpand }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            {/* --- ALWAYS VISIBLE SECTION (FIXED GRID) --- */}
            {/* --- BUG FIX: Changed md:grid-cols-3 to md:grid-cols-4 --- */}
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6">
                ...
                {isEditing ? (
                 ...
                 {/* This item was breaking the layout. Now it fits in the 4-col grid. */}
                 <ViewItem label="Employee Code" icon={<IdCardIcon className="h-4 w-4 mr-1 text-gray-400" />} value={user?.employeeCode} />
            </div>

            {/* --- COLLAPSIBLE SECTION --- */}
            {/* --- BUG FIX: Removed overflow-hidden to allow content to be seen --- */}
            <div className={`transition-all duration-500 ease-in-out ${isExpanded || isEditing ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="p-6 pt-0 space-y-8">
                    {/* Personal & Employment Info */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-6">
                        {isEditing ? (
                             <EditInput label="Middle Name" name="middleName" type="text" icon={<UserIcon className="h-4 w-4 mr-1 text-gray-400" />} value={formData.middleName} onChange={onFormChange} />
                        ) : (
                             <ViewItem label="Middle Name" icon={<UserIcon className="h-4 w-4 mr-1 text-gray-400" />} value={user?.middleName} />
                        )}
                        {isEditing ? (
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-center">
                <button
                    ...
                    className={`text-sm font-semibold text-indigo-600 hover:text-indigo-800 flex items-center transition-all ${isEditing ? 'opacity-0 h-0' : 'opacity-100 h-auto'}`}
                    disabled={isEditing}
                >
    ...
    return (
        <div className="space-y-8">
             <form onSubmit={handleSaveChanges}>
                ...
                {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-xl mb-4 animate-shake">{error}</div>}
                {success && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl mb-4 animate-fadeIn">{success}</div>}
