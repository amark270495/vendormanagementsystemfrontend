...
const getDeadlineClass = (dateString) => {
    ...
    try {
        ...
        if (isNaN(deadline.getTime())) return ''; // Return empty if date is invalid
        ...
        sevenDaysFromNow.setHours(0, 0, 0, 0); // Normalize seven days from now

        deadline.setHours(0, 0, 0, 0); // Normalize deadline to the start of the day
        ...
        if (this.state.hasError) {
            return (
                <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-md shadow-md error-boundary" role="alert">
                    <h2 className="font-bold text-lg mb-2">{this.props.title || "Something went wrong"}</h2>
...
const ReportsPage = ({ sheetKey }) => {
    ...
    const [filters, setFilters] = useState({
        sheetKey: sheetKey || 'taprootVMSDisplay',
        startDate: '',
        endDate: ''
    });
    ...
    const chartOptions = {
        ...
        plugins: { 
            legend: { position: 'top' }, 
            title: { display: false } // Title is handled by the h3 tag above the chart
        }
    };
    ...
    const renderChart = (chartType, data, title) => {
        // Defensive check: ensure data is a valid, non-empty object.
        if (!data || typeof data !== 'object' || Object.keys(data).length === 0) {
            return <div className="flex items-center justify-center h-full text-center text-gray-500 p-4">No data available for {title}.</div>;
        }
        const ChartComponent = { bar: Bar, pie: Pie, doughnut: Doughnut }[chartType];
        const chartData = getChartData(Object.keys(data), Object.values(data), '# of Jobs');
        
        return (
            <ErrorBoundary title={`Chart Error: ${title}`}>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => setEmailModalOpen(true)} className="px-4 py-2 bg-emerald-500 text-white rounded-md hover:bg-emerald-600" disabled={!reportData || loading || !canEmailReports}>
                            Email Report
                        </button>
                    </div>
                </div>
                ...
                {reportData && !loading && canViewReports ? (
