// ===================================================================================
// VMS Dashboard - React Application - FULLY INTEGRATED (Patched)
// ===================================================================================
...
window.onerror = function(message, source, lineno, colno, error) {
    ...
    if (rootElement && !rootElement.querySelector('.error-boundary')) { // Prevent duplicate messages
...
class ErrorBoundary extends Component {
    ...
    render() {
        if (this.state.hasError) {
            return (
                <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-md shadow-md error-boundary" role="alert">
                    <h2 className="font-bold text-lg mb-2">Something went wrong</h2>
                    <p>There was an error rendering this part of the application. Please try refreshing the page or contact support if the problem persists.</p>
                    {this.state.error && <pre className="mt-4 text-xs whitespace-pre-wrap">{this.state.error.toString()}</pre>}
                </div>
            );
        }
        ...
        <div className="relative" ref={node}>
            ...
            {isOpen && (
                <div className={`absolute right-0 mt-2 w-${width} bg-white rounded-md shadow-lg z-20 py-1`} onClick={(e) => {
                     if (e.target.closest('a, button')) { // Close on any link or button click inside dropdown
                        setIsOpen(false);
                    }
                }}>
                    {children}
...
// --- User Management Modals (No Changes) ---
const UserFormModal = ({ isOpen, onClose, onSave, userToEdit }) => {
    ...
    const [formData, setFormData] = useState({ displayName: '', username: '', password: '', userRole: 'Standard User', backendOfficeRole: 'Recruitment Team' });
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(false);
    useEffect(() => {
        if (userToEdit) setFormData({ displayName: userToEdit.displayName || '', username: userToEdit.username || '', password: '', userRole: userToEdit.userRole || 'Standard User', backendOfficeRole: userToEdit.backendOfficeRole || 'Recruitment Team' });
        else setFormData({ displayName: '', username: '', password: '', userRole: 'Standard User', backendOfficeRole: 'Recruitment Team' });
        setError('');
    }, [userToEdit, isOpen]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError('');
        if (!formData.displayName || !formData.username || (!userToEdit && !formData.password)) { setError("Please fill in all required fields (Display Name, Username, and Password for new users)."); return; }
        if (!userToEdit && formData.password.length < 6) { setError("Password must be at least 6 characters long."); return; }
        setLoading(true);
        try {
            await onSave(formData);
            onClose();
        } catch (err) { setError(`Failed to save user: ${err.message}`); } 
        finally { setLoading(false); }
    };
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={userToEdit ? "Edit User" : "Add New User"} size="lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label htmlFor="displayName" className="block text-sm font-medium text-gray-700">Display Name</label><input type="text" id="displayName" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value={formData.displayName} onChange={handleChange} required /></div>
                <div><label htmlFor="username" className="block text-sm font-medium text-gray-700">Username (Email)</label><input type="email" id="username" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value={formData.username} onChange={handleChange} required disabled={!!userToEdit} /></div>
                {!userToEdit && (<div><label htmlFor="password" className="block text-sm font-medium text-gray-700">Password</label><input type="password" id="password" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value={formData.password} onChange={handleChange} required={!userToEdit} /><p className="mt-1 text-sm text-gray-500">For new users, password must be at least 6 characters.</p></div>)}
                <div><label htmlFor="userRole" className="block text-sm font-medium text-gray-700">User Role</label><select id="userRole" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value={formData.userRole} onChange={handleChange} required>{userRoles.map(role => <option key={role} value={role}>{role}</option>)}</select></div>
                <div><label htmlFor="backendOfficeRole" className="block text-sm font-medium text-gray-700">Backend Office Role</label><select id="backendOfficeRole" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" value={formData.backendOfficeRole} onChange={handleChange} required>{backendOfficeRoles.map(role => <option key={role} value={role}>{role}</option>)}</select></div>
                <div className="flex justify-end space-x-2"><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 flex items-center justify-center" disabled={loading}>{loading ? <Spinner size="5" /> : 'Save User'}</button></div>
            </form>
        </Modal>
    );
};
const DeleteUserModal = ({ isOpen, onClose, onConfirm, userToDelete }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Confirm Delete" size="sm">
            ...
            <div className="flex justify-end space-x-2"><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button type="button" onClick={handleDelete} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center justify-center" disabled={loading}>{loading ? <Spinner size="5" /> : 'Delete'}</button></div>
        </Modal>
    );
};
const AdminPage = () => {
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    <div className="flex justify-end mb-4"><button onClick={handleAddClick} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 mr-2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><line x1="19" y1="8" x2="19" y2="14"></line><line x1="22" y1="11" x2="16" y2="11"></line></svg>Add User</button></div>
                    {loading && <div className="flex justify-center items-center h-64"><Spinner /></div>}
                    {error && <div className="text-red-500 bg-red-100 p-4 rounded-lg">Error: {error}</div>}
                    {!loading && !error && (<div className="overflow-x-auto">{users.length > 0 ? (<table className="w-full text-sm text-left text-gray-500"><thead className="text-xs text-gray-700 uppercase bg-gray-50"><tr><th scope="col" className="px-6 py-3">Display Name</th><th scope="col" className="px-6 py-3">Username</th><th scope="col" className="px-6 py-3">Role</th><th scope="col" className="px-6 py-3">Backend Role</th><th scope="col" className="px-6 py-3">Actions</th></tr></thead><tbody>{users.map((u) => (<tr key={u.username} className="bg-white border-b hover:bg-gray-50"><td className="px-6 py-4 font-medium text-gray-900">{u.displayName}</td><td className="px-6 py-4">{u.username}</td><td className="px-6 py-4">{u.userRole}</td><td className="px-6 py-4">{u.backendOfficeRole}</td><td className="px-6 py-4 flex space-x-2"><button onClick={() => handleEditClick(u)} className="text-indigo-600 hover:text-indigo-900 p-1 rounded-full hover:bg-gray-100" aria-label={`Edit user ${u.displayName}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button><button onClick={() => handleDeleteClick(u)} className="text-red-600 hover:text-red-900 p-1 rounded-full hover:bg-gray-100" aria-label={`Delete user ${u.displayName}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></button></td></tr>))}</tbody></table>) : (<p className="text-center text-gray-500 p-4">No users found.</p>)}</div>)}
                </div>
            </div>
...
const HomePage = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {loading && <div className="flex justify-center items-center h-64"><Spinner /></div>}
            {error && <div className="text-red-500 bg-red-100 p-4 rounded-lg">Error: {error}</div>}
            {!loading && !error && (Object.keys(data).length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{Object.entries(data).map(([assignee, jobs]) => (<div key={assignee} className="bg-white p-6 rounded-lg shadow-md border"><h2 className="text-xl font-semibold text-indigo-700 mb-4 border-b pb-2">{assignee}</h2><div className="space-y-3 max-h-80 overflow-y-auto">{jobs.map(job => (<div key={job.postingId} className="p-3 bg-slate-50 rounded-md"><p className="font-bold text-gray-800">{job.jobTitle}</p><p className="text-sm text-gray-600">{job.clientName}</p><p className={`text-sm font-medium ${getDeadlineClass(job.deadline)}`}>Deadline: {formatDate(job.deadline)}</p></div>))}</div></div>))}</div>) : (<p className="text-center text-gray-500 p-4">No open job postings found.</p>))}
        </div>
    );
};

// --- Job Posting Form Page (No Changes) ---
const JobPostingFormPage = ({ onFormSubmit }) => {
    ...
    const formFields = [{ name: 'Posting ID', type: 'text', required: true }, { name: 'Posting Title', type: 'text', required: true }, { name: 'Posting Date', type: 'date', required: true }, { name: 'Last Submission Date', type: 'date', required: true }, { name: 'Max Submissions', type: 'number', required: true }, { name: 'Max C2C Rate', type: 'text', required: true }, { name: 'Client Name', type: 'text', required: true }, { name: 'Company Name', type: 'select', required: true, options: ['Eclat Solutions LLC', 'Taproot Solutions INC'] }, { name: 'Posting From', type: 'select', required: true, options: postingFromOptions }, { name: 'Work Location', type: 'text', required: true }, { name: 'Work Position Type', type: 'text', required: true }, { name: 'Required Skill Set', type: 'textarea', required: true }, { name: 'Any Required Certificates', type: 'textarea' },];
    const handleChange = (e) => setFormData(prev => ({ ...prev, [e.target.name]: e.target.value }));
    const handleSubmit = async (e) => {
        e.preventDefault();
        setError(''); setSuccess(''); setLoading(true);
        const canAddPosting = user.userRole.includes('Admin') || user.backendOfficeRole.includes('Data Entry') || user.backendOfficeRole.includes('Data Entry & Viewer');
        if (!canAddPosting) { setError("You do not have permission to add new job postings."); setLoading(false); return; }
        try {
            const result = await api.processJobPosting(formData, user.userIdentifier);
            if (result.success) {
                setSuccess(result.message); setFormData({});
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-8 rounded-lg shadow-lg">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {formFields.map(field => (<div key={field.name} className={field.type === 'textarea' ? 'md:col-span-2' : ''}><label className="block text-sm font-medium text-gray-700">{field.name} {field.required && '*'}</label>{field.type === 'textarea' ? (<textarea name={field.name} value={formData[field.name] || ''} onChange={handleChange} required={field.required} rows="4" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"></textarea>) : field.type === 'select' ? (<select name={field.name} value={formData[field.name] || ''} onChange={handleChange} required={field.required} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"><option value="">Select an option</option>{field.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>) : (<input type={field.type} name={field.name} value={formData[field.name] || ''} onChange={handleChange} required={field.required} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" />)}</div>))}
                    <div className="md:col-span-2 flex justify-end"><button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg flex items-center justify-center" disabled={loading}>{loading ? <Spinner size="5" /> : 'Submit Job'}</button></div>
                </form>
            </div>
        </div>
    );
...
const ReportsPage = ({ sheetKey }) => {
    const { user } = useAuth();
    const [reportData, setReportData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const [filters, setFilters] = useState({
        sheetKey: sheetKey || 'taprootVMSDisplay', // Use passed sheetKey or default
        startDate: '',
        endDate: ''
    });
    const [isEmailModalOpen, setEmailModalOpen] = useState(false);

    // Update filters if the sheetKey prop from navigation changes
    useEffect(() => {
    ...
    }, [filters, user?.userIdentifier, canViewReports]); // Depend on filters to refetch when they change

    // Automatically generate report when the component loads or filters change
    useEffect(() => {
        generateReport();
    }, [generateReport]);
    ...
    const renderChart = (chartType, data, title) => {
        ...
        const ChartComponent = { bar: Bar, pie: Pie, doughnut: Doughnut }[chartType];
        return <ChartComponent options={{...chartOptions, title: {display: false}}} data={getChartData(Object.keys(data), Object.values(data), '# of Jobs')} />;
    };

    return (
        <>
        <div className="space-y-6">
            <h1 className="text-3xl font-bold text-gray-800">Reports - {DASHBOARD_CONFIGS[filters.sheetKey]?.title || ''}</h1>
            <div className="bg-white p-4 rounded-lg shadow-sm border flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-4">
                    {/* The sheetKey is now controlled by the TopNav, so we can hide this selector */}
                ...
                <div className="flex items-center space-x-2">
                    <button onClick={generateReport} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700" disabled={loading || !canViewReports}>
                        {loading ? <Spinner size="5" /> : 'Refresh Report'}
                    </button>
                ...
                {reportData && canViewReports ? (
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="bg-white p-6 rounded-lg shadow-lg border flex flex-col items-center" style={{ height: '400px' }}><h3 className="font-semibold text-lg text-gray-800 mb-4">Jobs by Client</h3><div className="relative w-full h-full">{renderChart('bar', reportData.clientJobCounts, 'Jobs by Client')}</div></div>
                            <div className="bg-white p-6 rounded-lg shadow-lg border flex flex-col items-center" style={{ height: '400px' }}><h3 className="font-semibold text-lg text-gray-800 mb-4">Jobs by Position Type</h3><div className="relative w-full h-full">{renderChart('pie', reportData.positionTypeCounts, 'Jobs by Position Type')}</div></div>
                            <div className="bg-white p-6 rounded-lg shadow-lg border lg:col-span-2 flex flex-col items-center" style={{ height: '400px' }}><h3 className="font-semibold text-lg text-gray-800 mb-4">Jobs by Assignee</h3><div className="relative w-full h-full">{renderChart('doughnut', reportData.workingByCounts, 'Jobs by Assignee')}</div></div>
                        </div>
                    </div>
                ) : (
                    !loading && !error && <p className="text-center text-gray-500 p-4">No report data found for the selected criteria.</p>
                )}
            </ErrorBoundary>
        </div>
        <EmailReportModal 
...
const EmailReportModal = ({ isOpen, onClose, sheetKey }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Email Job Report" size="lg">
            ...
            {success && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">{success}</div>}
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="block text-sm font-medium text-gray-700">To (comma-separated)</label><input type="text" value={toEmails} onChange={e => setToEmails(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" required /></div>
                <div><label className="block text-sm font-medium text-gray-700">CC (comma-separated)</label><input type="text" value={ccEmails} onChange={e => setCcEmails(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" /></div>
                <div><label className="block text-sm font-medium text-gray-700">Job Status to Include</label><select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"><option value="all">All</option><option value="open">Open</option><option value="closed">Closed</option></select></div>
                <div className="flex justify-end space-x-2"><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 flex items-center justify-center" disabled={loading}>{loading ? <Spinner size="5" /> : 'Send Email'}</button></div>
            </form>
        </Modal>
    );
};
...
const HeaderMenu = ({ header, sortConfig, onSort, filterConfig, onFilterChange }) => {
    ...
    return (
        <div className="p-2 space-y-2">
            <div>
                <button onClick={() => onSort('ascending')} className="w-full text-left px-2 py-1 text-sm rounded hover:bg-slate-100 close-on-click">Sort Ascending</button>
                <button onClick={() => onSort('descending')} className="w-full text-left px-2 py-1 text-sm rounded hover:bg-slate-100 close-on-click">Sort Descending</button>
            </div>
            <hr/>
            <div className="space-y-2">
                <label className="text-xs font-semibold text-gray-500 px-2">FILTER</label>
                <select value={filterType} onChange={e => setFilterType(e.target.value)} className="w-full p-1 border rounded text-sm">{getFilterOptions().map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select>
...
// --- Dashboard Page and Components (No Changes) ---
const DashboardPage = ({ sheetKey }) => {
    ...
    const canEditDashboard = user?.userRole?.includes('Admin') || user?.backendOfficeRole?.includes('Data Entry') || user?.backendOfficeRole?.includes('Data Entry & Viewer');
    ...
    const transformedData = useMemo(() => {
        ...
        const workLocationIndex = originalHeaderMap['Work Location'];
        ...
        if (clientIndex !== undefined && postingFromIndex !== undefined && workLocationIndex !== undefined && clientInfoIndex !== -1) {
            ...
            transformedHeader = transformedHeader.filter(h => h !== 'Posting From' && h !== 'Work Location');
        }
        ...
        const transformedRows = rows.map(row => {
            ...
            transformedHeader.forEach(newHeaderName => {
                if (newHeaderName === 'Client Info') {
                    ...
                    const workLocation = row[workLocationIndex] || '';
                    let clientInfo = client;
                    if (postingFrom && postingFrom !== 'All' && postingFrom !== 'Need To Update') clientInfo += ` / ${postingFrom}`;
                    if (workLocation && workLocation !== 'Need To Update') clientInfo += ` (${workLocation})`;
    ...
    const handleCellEdit = (rowIndex, cellIndex, value) => {
        if (!canEditDashboard) { setError("You do not have permission to edit dashboard data."); return; }
    ...
    const handleSaveChanges = async () => {
        if (!canEditDashboard) { setError("You do not have permission to save dashboard changes."); return; }
    ...
    const handleAction = async (actionType, job) => {
        const isAuthorizedForActions = user?.userRole?.includes('Admin') || user?.backendOfficeRole?.includes('Data Entry') || user?.backendOfficeRole?.includes('Data Entry & Viewer');
        if (!isAuthorizedForActions) { setError("You do not have permission to perform this action on job postings."); setModalState({ type: null, job: null }); return; }
        setLoading(true);
    ...
    return (
        <div className="space-y-4">
            <div className="bg-white p-4 rounded-lg shadow-sm border"><h2 className="text-xl font-bold text-gray-800">{DASHBOARD_CONFIGS[sheetKey]?.title || 'Dashboard'}</h2></div>
            <div className="bg-white p-4 rounded-lg shadow-sm border flex flex-wrap items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-4"><input type="text" placeholder="Search all columns..." value={generalFilter} onChange={(e) => setGeneralFilter(e.target.value)} className="shadow-sm border-gray-300 rounded-md px-3 py-2" /><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="shadow-sm border-gray-300 rounded-md px-3 py-2"><option value="">All Statuses</option><option value="Open">Open</option><option value="Closed">Closed</option></select></div>
                <div className="flex items-center space-x-2">{Object.keys(unsavedChanges).length > 0 && (<button onClick={handleSaveChanges} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex items-center justify-center" disabled={loading}>{loading ? <Spinner size="5" /> : 'Save Changes'}</button>)}<Dropdown trigger={<button className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 flex items-center">Options<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-2"><polyline points="6 9 12 15 18 9"></polyline></svg></button>}><a href="#" onClick={(e) => {e.preventDefault(); setColumnModalOpen(true);}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Column Settings</a><a href="#" onClick={(e) => {e.preventDefault(); downloadPdf();}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Download PDF</a><a href="#" onClick={(e) => {e.preventDefault(); downloadCsv();}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Download CSV</a></Dropdown></div>
            </div>
            {loading && <div className="flex justify-center items-center h-64"><Spinner /></div>}
            {error && <div className="text-red-500 bg-red-100 p-4 rounded-lg">Error: {error}</div>}
            {!loading && !error && (filteredAndSortedData.length > 0 ? (<div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-y-auto relative" style={{ maxHeight: '70vh' }}><table className="w-full text-sm text-left text-gray-500 table-layout"><thead className="text-xs text-gray-700 uppercase bg-slate-200 sticky top-0 z-10"><tr>{displayHeader.map(h => { const isSkillColumn = h === 'Required Skill Set'; const isTitleColumn = h === 'Posting Title'; let style = { minWidth: '140px' }; if (isSkillColumn) style = { width: '300px' }; else if (isTitleColumn) style = { width: '200px' }; return (<th key={h} scope="col" className="p-0" style={style}><Dropdown width="64" trigger={<div className="header-menu-trigger font-bold"><span>{h} {sortConfig.key === h ? (sortConfig.direction === 'ascending' ? '▲' : '▼') : ''}</span></div>}><HeaderMenu header={h} sortConfig={sortConfig} onSort={(dir) => handleSort(h, dir)} filterConfig={columnFilters[h]} onFilterChange={handleColumnFilterChange}/></Dropdown></th>);})}<th scope="col" className="px-4 py-3" style={{width: '80px'}}>Action</th></tr></thead><tbody>{filteredAndSortedData.map((row, rowIndex) => (<tr key={row[0] || rowIndex} className="bg-white border-b hover:bg-gray-50">{row.map((cell, cellIndex) => { const headerName = displayHeader[cellIndex]; const isEditable = canEditDashboard && EDITABLE_COLUMNS.includes(headerName); const isDate = DATE_COLUMNS.includes(headerName); const postingId = row[displayHeader.indexOf('Posting ID')]; const isModified = unsavedChanges[postingId] && unsavedChanges[postingId][headerName] !== undefined; const deadlineClass = headerName === 'Deadline' ? getDeadlineClass(cell) : ''; return (<td key={cellIndex} className={`px-4 py-3 text-center align-middle ${isModified ? 'modified-cell' : ''} ${deadlineClass}`} contentEditable={isEditable} suppressContentEditableWarning={true} onFocus={e => { if (!isEditable) return; if (e.target.innerText === 'Need To Update') e.target.innerText = ''; }} onBlur={e => { if (!isEditable) return; let currentValue = e.target.innerText.trim(); if (currentValue === '') { currentValue = 'Need To Update'; e.target.innerText = 'Need To Update'; } handleCellEdit(rowIndex, cellIndex, currentValue); }}>{isDate ? formatDate(cell) : cell}</td>);})}<td className="px-4 py-3">{canEditDashboard ? (<ActionMenu job={row} onAction={(type, job) => setModalState({type, job: jobToObject(job)})} />) : (<button className="text-gray-400 p-1 rounded-full cursor-not-allowed" aria-label="No actions available"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><circle cx="12" cy="12" r="1"></circle><circle cx="12" cy="5" r="1"></circle><circle cx="12" cy="19" r="1"></circle></svg></button>)}</td></tr>))}</tbody></table></div>) : (!loading && !error && <p className="text-center text-gray-500 p-4">No job postings found matching your filters.</p>))}
...
const ColumnSettingsModal = ({ isOpen, onClose, allHeaders, userPrefs, onSave }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Column Settings" size="lg">
            ...
            <div className="space-y-2">{order.map((header, index) => (<div key={header} className="flex items-center justify-between p-2 bg-gray-50 rounded-md border cursor-grab column-list-item" draggable onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={(e) => e.target.classList.remove('dragging')} onDragOver={(e) => e.preventDefault()} onDrop={handleDrop}><span>{header}</span><input type="checkbox" checked={visibility[header] || false} onChange={() => handleVisibilityChange(header)} className="h-5 w-5 rounded"/></div>))}</div>
            <div className="flex justify-end mt-6 space-x-2"><button onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button onClick={handleSave} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Save Settings</button></div>
        </Modal>
    );
};
const ActionMenu = ({ job, onAction }) => (<Dropdown trigger={<button className="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100" aria-label="Job actions menu"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><circle cx="12" cy="12" r="1"></circle><circle cx="12" cy="5" r="1"></circle><circle cx="12" cy="19" r="1"></circle></svg></button>}><a href="#" onClick={(e) => {e.preventDefault(); onAction('details', job)}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">View Details</a><a href="#" onClick={(e) => {e.preventDefault(); onAction('close', job)}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Close Job</a><a href="#" onClick={(e) => {e.preventDefault(); onAction('archive', job)}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Archive Job</a><a href="#" onClick={(e) => {e.preventDefault(); onAction('delete', job)}} className="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100">Delete Job</a></Dropdown>);
const ConfirmationModal = ({ isOpen, onClose, onConfirm, title, message, confirmText }) => {
    ...
    return (<Modal isOpen={isOpen} onClose={onClose} title={title} size="sm"><p className="text-gray-600">{message}</p><div className="flex justify-end mt-6 space-x-2"><button onClick={onClose} className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Cancel</button><button onClick={onConfirm} className={`px-4 py-2 text-white rounded-md ${confirmButtonClasses[confirmText] || 'bg-indigo-600'}`}>{confirmText}</button></div></Modal>);
};
const ViewDetailsModal = ({ isOpen, onClose, job }) => (<Modal isOpen={isOpen} onClose={onClose} title="Job Details" size="lg">{job && <div className="space-y-4">{Object.entries(job).map(([key, value]) => (<div key={key}><h4 className="text-sm font-medium text-gray-500">{key}</h4><p className="text-gray-800">{String(DATE_COLUMNS.includes(key) ? formatDate(value) : value)}</p></div>))}</div>}</Modal>);

// --- Messages Page Component (No Changes) ---
const MessagesPage = () => {
    const { user } = useAuth();
    const [users, setUsers] = useState([]);
    ...
    const handleSendMessage = async (e) => {
        ...
        setNewMessage('');
        try {
            const tempMessage = { sender: user.userIdentifier, recipient: selectedRecipient.username, messageContent: messageToSend, timestamp: new Date().toISOString(), id: Date.now() };
            ...
            if (!result.success) { setError(result.message); setMessages(prev => prev.filter(msg => msg.id !== tempMessage.id)); }
        } catch (err) { setError(`Failed to send message: ${err.message}`); setMessages(prev => prev.filter(msg => msg.id !== tempMessage.id)); }
    };
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row bg-white rounded-lg shadow-lg h-[70vh]">
                <div className="w-full md:w-1/4 border-r border-gray-200 p-4 overflow-y-auto"><h2 className="text-lg font-semibold text-gray-700 mb-4">Users</h2>{loadingUsers && <Spinner size="6" />}{error && <div className="text-red-500 text-sm">{error}</div>}{!loadingUsers && users.length === 0 && <p className="text-gray-500 text-sm">No other users found.</p>}<ul className="space-y-2">{users.map(u => (<li key={u.username}><button onClick={() => setSelectedRecipient(u)} className={`w-full text-left p-3 rounded-md transition-colors duration-200 ${selectedRecipient?.username === u.username ? 'bg-emerald-100 text-emerald-800 font-semibold' : 'hover:bg-gray-100 text-gray-700'}`}><p className="font-medium">{u.displayName}</p><p className="text-xs text-gray-500 break-all">ID: {u.userIdentifier}</p></button></li>))}</ul></div>
                <div className="flex-1 flex flex-col">{selectedRecipient ? (<><div className="p-4 border-b border-gray-200 bg-gray-50"><h2 className="text-lg font-semibold text-gray-800">Chat with {selectedRecipient.displayName}</h2><p className="text-sm text-gray-500 break-all">ID: {selectedRecipient.userIdentifier}</p></div><div className="flex-1 p-4 overflow-y-auto space-y-4 bg-gray-50">{loadingMessages && <div className="flex justify-center"><Spinner size="6" /></div>}{messages.length === 0 && !loadingMessages && <p className="text-gray-500 text-center">No messages yet. Start a conversation!</p>}{messages.map((msg, index) => (<div key={msg.id || index} className={`flex ${msg.sender === user.userIdentifier ? 'justify-end' : 'justify-start'}`}><div className={`max-w-xs p-3 rounded-lg shadow-md ${msg.sender === user.userIdentifier ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-800'}`}><p className="text-xs font-semibold mb-1">{msg.sender === user.userIdentifier ? 'You' : selectedRecipient.displayName}</p><p>{msg.messageContent}</p><p className="text-right text-xs mt-1 opacity-75">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>))}<div ref={messagesEndRef} /></div><form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 bg-white flex items-center space-x-2"><input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type your message..." className="flex-1 border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" disabled={loadingMessages} /><button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md disabled:bg-gray-400" disabled={!newMessage.trim() || loadingMessages}>Send</button></form></>) : (<div className="flex-1 flex items-center justify-center text-gray-500">Select a user to start a conversation.</div>)}</div>
            </div>
        </div>
    );
};
...
const TopNav = ({ user, logout, currentPage, setCurrentPage }) => {
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        } catch (err) { setError('Could not fetch notifications'); }
    }, [user.userIdentifier]);
    ...
    const handleMarkAsRead = async (notifsToMark) => {
        try {
            await api.markNotificationsAsRead(notifsToMark.map(n => n.id), user.userIdentifier);
            fetchNotifications();
                ...
                <div className="flex justify-between items-center h-16">
                    <div className="flex items-center space-x-8">
                        ...
                        <nav className="hidden md:flex space-x-1">
                            ...
                            {(user?.userRole?.includes('Admin') || user?.backendOfficeRole?.includes('Data Viewer') || user?.backendOfficeRole?.includes('Data Entry') || user?.backendOfficeRole?.includes('Data Entry & Viewer') || user?.backendOfficeRole?.includes('Recruitment Manager') || user?.backendOfficeRole?.includes('Recruitment Team')) && (
                                <Dropdown trigger={<button className={`px-3 py-2 rounded-md text-sm font-medium flex items-center ${currentPage.type === 'dashboard' ? 'text-indigo-600 bg-indigo-50' : 'text-gray-500 hover:text-gray-700'}`}>Dashboards <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-1"><polyline points="6 9 12 15 18 9"></polyline></svg></button>}>
                            ...
                            {/* MODIFIED: Reports is now a dropdown */}
                            {canAccessReportsNav && (
                                <Dropdown trigger={<button className={`px-3 py-2 rounded-md text-sm font-medium flex items-center ${currentPage.type === 'reports' ? 'text-indigo-600 bg-indigo-50' : 'text-gray-500 hover:text-gray-700'}`}>Reports <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-1"><polyline points="6 9 12 15 18 9"></polyline></svg></button>}>
                                    {Object.entries(DASHBOARD_CONFIGS).map(([key, config]) => (
                                        <a href="#" key={key} onClick={(e) => {e.preventDefault(); setCurrentPage({type: 'reports', key})}} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{config.title}</a>
                                    ))}
                                </Dropdown>
                            )}
                    ...
                    <div className="flex items-center space-x-4">
                        <Dropdown 
                            ...
                            <div className="p-2">
                                <div className="flex justify-between items-center mb-2 px-2"><h4 className="font-semibold text-gray-800">Notifications</h4><button onClick={() => handleMarkAsRead(notifications)} className="text-xs text-indigo-600 hover:underline">Mark all as read</button></div>
                                <div className="max-h-80 overflow-y-auto">{notifications.length > 0 ? notifications.map(n => (<div key={n.id} className="p-2 border-b hover:bg-slate-50"><p className="text-sm text-gray-700">{n.message}</p><p className="text-xs text-gray-400">{new Date(n.timestamp).toLocaleString()}</p></div>)) : <p className="text-sm text-gray-500 p-4 text-center">No new notifications.</p>}</div>
                            </div>
                        </Dropdown>

                        <Dropdown trigger={<button className="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" aria-label="User menu"><span className="sr-only">Open user menu</span><div className="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 text-gray-600"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg></div></button>}>
                            <div className="px-4 py-2 border-b"><p className="text-sm font-medium text-gray-900">{user.userName}</p><p className="text-sm text-gray-500 truncate">{user.userIdentifier}</p></div>
...
const MainApp = () => {
    // MODIFIED: Default to the 'home' page
    const [currentPage, setCurrentPage] = useState({type: 'home'});
    const { user, logout } = useAuth();
    
    const renderPage = () => {
        const { type, key } = currentPage;

        const isAdmin = user?.userRole?.includes('Admin');
        const isDataEntry = user?.backendOfficeRole?.includes('Data Entry') || user?.backendOfficeRole?.includes('Data Entry & Viewer');
        const isDataViewer = user?.backendOfficeRole?.includes('Data Viewer') || user?.backendOfficeRole?.includes('Data Entry') || user?.backendOfficeRole?.includes('Data Entry & Viewer') || user?.backendOfficeRole?.includes('Recruitment Manager') || user?.backendOfficeRole?.includes('Recruitment Team');
        const canAccessReports = isAdmin || user?.backendOfficeRole?.includes('Recruitment Manager') || user?.backendOfficeRole?.includes('Data Viewer') || user?.backendOfficeRole?.includes('Data Entry & Viewer');

        switch(type) {
            case 'home': return <HomePage />;
            case 'messages': return <MessagesPage />;
            case 'dashboard': 
                if (isDataViewer || isAdmin) return <DashboardPage sheetKey={key} />;
            ...
            case 'reports': 
                // MODIFIED: Pass the sheetKey to the ReportsPage
                if (canAccessReports) return <ReportsPage sheetKey={key} />;
